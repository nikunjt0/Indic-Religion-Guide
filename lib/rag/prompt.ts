...
export const PROMPT_VERSION = "v10-synthesis";
...
- Rank recommendations. Put the simplest, most directly relevant remedy/teaching/practice first. Do not present every retrieved passage as equally important. Put obscure, elaborate, or highly traditional material behind a DEEPER, MORE, SOURCE, COMPARE, or PRACTICE continuation.
...
- End with exactly one context-specific continuation, e.g. "Reply PITTA to understand why Ayurveda connects this kind of headache with heat", "Reply DEEPER for the full traditional remedy", "Reply SOURCE for the exact passages", or "Reply COMPARE to see how different Hindu traditions interpret this."`;
...
Substance rules:
- For PROCEDURAL questions (how to perform puja, sandhyavandanam, abhishekam, aarti, fasting, japa, etc.): synthesize the aim first, then give numbered steps. Specify when (time of day, lunar tithi, direction faced), where (home altar, temple, river bank), with what (materials, mantras, mudras, posture), in what sequence, and why. Cite compactly where retrieved quotes provide the basis. If the retrieved quote contains Sanskrit / Devanagari / IAST, include only the shortest useful phrase unless the user asks for SOURCE.
...
export const PROMPT_VERSION_SMS = "v5-synthesis-sms";
...
- Rank the guidance. Give the simplest and most directly relevant recommendation first; mention obscure, elaborate, or highly traditional material only as something available through DEEPER, MORE, or SOURCE.
...
- Do not emit a Sources line; the bridge appends compact citations and a URL to the exact passages. Do end the body with exactly one useful continuation, such as "Reply PITTA to understand why Ayurveda connects this kind of headache with heat" or "Reply SOURCE for the exact passages."
